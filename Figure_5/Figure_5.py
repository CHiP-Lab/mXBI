...
"An automated, touchscreen-based system for auditory training and subj_dfing of common marmosets"
...
result_filename = os.path.join((Path('../')).resolve(), 'data_analysis/analysis_output', 'Figure_5.txt')
...
axs[0].set_title('Animal a', fontsize=12)
axs[1].set_title('Animal b', fontsize=12)
axs[2].set_title('Animal d', fontsize=12)

axs[0].text(thrs[0] + 2, 0.05, thrs[0], fontsize=12)
axs[1].text(thrs[1] + 2, 0.05, thrs[1], fontsize=12)
axs[2].text(thrs[2] + 2, 0.05, thrs[2], fontsize=12)

axs[0].text(-2, .8, "{}{}".format('N = ', ntrs[0]), fontsize=10)
axs[1].text(-2, .8, "{}{}".format('N = ', ntrs[1]), fontsize=10)
axs[2].text(-2, .8, "{}{}".format('N = ', ntrs[2]), fontsize=10)
...
axs[0].set_xlabel('Amplitude (dB SPL)', fontsize=12)
axs[0].set_ylabel('Proportion correct', fontsize=12)

f.tight_layout()

AMP_df.groupby(['animal', 'level'])['total'].sum()
