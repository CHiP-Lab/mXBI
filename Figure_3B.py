...
- Figure_3B.csv, Figure_3B.txt
- Figure_3B_dPrime.txt, Figure_3B_dPrime.csv
- Figure_3B_HR_binomial.txt, Figure_3B_HR_binomial.csv
- Figure_3B_RTs.txt, Figure_3B_RTs.csv
...
def dataload_dpDF():
    dP_df = pd.read_csv('./data/Figure_3B_dPDF.csv', low_memory=False, decimal=',')
...
def dataload_HR():
    HR = pd.read_csv('./data/Figure_3B_HR.csv', low_memory=False, decimal=',')
...
def dataload_Responses():
    Responses = pd.read_csv('./data/Figure_3B_Responses.csv', low_memory=False, decimal=',')
...
# =============================================
# FIGURE 3B
figure3B_height = (80 / 25.4) * sizeMult
figure3B_width = (180 / 25.4) * sizeMult
...
g, ax = plt.subplots(2, len(monkeys_list), sharey='row', sharex='col', constrained_layout=True,
                     gridspec_kw={'height_ratios': [1, 2]}, figsize=(figure3B_width, figure3B_height))
...
for m in range(0, len(monkeys_list)):
    ...
    if m < 9:
        ...
        if p < 0.05:
            ax[m + len(monkeys_list)].text(.5, 0.5, '*', color='black', fontsize=12, va="top", ha="center")
...
# save the plot
if saveplot:
    plt.savefig('./analysis_output/Figure_3B.pdf', format='pdf')
    plt.savefig('./analysis_output/Figure_3B.png', format='png')
    plt.close()
...
# save the tables
if savetable:
    STATS.to_csv(r'./analysis_output/Figure_3B_RTs.csv', sep=';', decimal=".", index=False)
    STATS.to_csv(r'./analysis_output/Figure_3B_RTs.txt', sep=';', decimal=".", index=False)

    HR.to_csv(r'./analysis_output/Figure_3B_HR_binomial.csv', sep=';', decimal=".", index=False)
    HR.to_csv(r'./analysis_output/Figure_3B_HR_binomial.txt', sep=';', decimal=".", index=False)

    dP_df.to_csv(r'./analysis_output/Figure_3B_dPrime.csv', sep=';', decimal=".", index=False)
    dP_df.to_csv(r'./analysis_output/Figure_3B_dPrime.txt', sep=';', decimal=".", index=False)
