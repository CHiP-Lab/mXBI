...
if saveplot:
    ...
    for s in range(0, len(sounds)):
        ...
        ax[s].set_title(sounds[s].split('/')[1][0:-1 - 3], fontsize=titleFontSize)

        ax[s].plot(signalData)
        ax[s + 5].specgram(signalData, Fs=samplingFrequency, cmap='jet')

        if s + 5 == 5:
            ax[s + 5].set_ylabel('Frequency')
            ax[s + 5].set_xlabel('Time')

        if s == 0:
            ax[s].set_ylabel('Amplitude')
...
# Load the data
def dataload():
    performance_df = pd.read_csv('./data/Figure_3B.csv', low_memory=False, decimal=',')
...
figure4B_height = (180 / 25.4) * sizeMult
figure4B_width = (60 / 25.4) * sizeMult
...
for t in range(0, len(tasks_list)):
    ...
    if t == 0:
        ax[t].set_title(tasks_name[t], fontsize=titleFontSize)
        ax[t].set(xlabel=None, ylabel='Hit Rate')
    ...
    if t == 3:
        idx = [5, 6, 8, 11]
        ax[t].set_title(tasks_name[t], fontsize=titleFontSize)
        ax[t].set(xlabel='Percentage of Trials', ylabel='Hit Rate')
    ...
    for i in idx:
        l.append(labels[i])
        h.append(handles[i])
    g.legend(h, l, loc='lower center', ncol=2, columnspacing=0.5, frameon=True, title=None)
